 ...
 *   RomaClientFactory fact = new RomaClientFactory();
 *   RomaClient client = fact.createRomaClient();
 *   BufferedImageAppender biAppender = new BufferedImageAppender(client);
 *   // open connections with ROMA
 *   client.open(initNodes);
 *   // set a format
 *   biAppender.setFormat(&quot;jpg&quot;);
 *   // execute a get command
 *   biAppender.put(fileName, img);
 *   // execute a put command
 *   BufferedImage newImg = biAppender.get(fileName);
...
public class BufferedImageUtil {
  ...
  public BufferedImageUtil(RomaClient client) {
    this.client = client;
  }
